<!-- Page administration des commentaires. -->
...
//Les informations sur chaque commentaires sont stockées dans leur propre tableau dans le tableau principal $comments. La fonction storey_sort() permet de classer par ordre décroissant. La fonction usort permet de trier les valeurs dans un tableau multidimensionnel basé sur un champ particulier.
function storey_sort($comment_a, $comment_b){
	return $comment_b['signalement'] - $comment_a['signalement'];
}
usort($comments, "storey_sort");

//On affiche ligne par ligne les commentaires présents en bdd.
foreach ($comments as $comment) :
	...
	<tr>
		...
		<?php if ( $comment['signalement'] === '1' ) : ?>
			<td class="rouge">Commentaire signalé</td>
		<?php else : ?>
			<td> </td>
		<?php endif; ?>
		<td>
